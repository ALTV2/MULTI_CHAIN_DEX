...
describe("CrossChainOrderBook", function () {
  ...
  const ETHEREUM_CHAIN_ID = 11155111n; // Sepolia
  const ONE_HOUR = 3600;
  const ONE_DAY = 86400;
  ...
  beforeEach(async function () {
    ...
    await ccob.addSupportedChain(ETHEREUM_CHAIN_ID);
  });
  ...
  describe("Chain Management", function () {
    it("should add supported chain", async function () {
      const newChainId = 137n; // Polygon Mainnet
    ...
    it("should remove supported chain", async function () {
      await expect(ccob.removeSupportedChain(ETHEREUM_CHAIN_ID))
        .to.emit(ccob, "ChainRemoved")
        .withArgs(ETHEREUM_CHAIN_ID);

      expect(await ccob.supportedChains(ETHEREUM_CHAIN_ID)).to.be.false;
    });
    ...
    it("should only allow owner to remove chains", async function () {
      await expect(
        ccob.connect(alice).removeSupportedChain(ETHEREUM_CHAIN_ID)
      ).to.be.revertedWithCustomError(ccob, "OwnableUnauthorizedAccount");
    });
    ...
    it("should create a cross-chain order successfully", async function () {
      ...
      await expect(
        ccob.connect(alice).createOrder(
          ...
          ETHEREUM_CHAIN_ID, // targetChainId
      ...
      expect(order.targetChainId).to.equal(ETHEREUM_CHAIN_ID);
    ...
    it("should revert if expiry is in the past", async function () {
      ...
      await expect(
        ccob.connect(alice).createOrder(
          ethers.ZeroAddress, ethers.parseEther("1"),
          ethers.ZeroAddress, ethers.parseEther("0.1"),
          ETHEREUM_CHAIN_ID, alice.address, MIN_TIMELOCK, pastExpiry
        )
    ...
    it("should revert if timelock is less than 1 hour", async function () {
      ...
      await expect(
        ccob.connect(alice).createOrder(
          ethers.ZeroAddress, ethers.parseEther("1"),
          ethers.ZeroAddress, ethers.parseEther("0.1"),
          ETHEREUM_CHAIN_ID, alice.address, shortTimelock, expiresAt
        )
  ...
  describe("View Functions", function () {
    ...
    it("should return active orders for target chain", async function () {
      const orders = await ccob.getActiveOrdersForTargetChain(ETHEREUM_CHAIN_ID);
