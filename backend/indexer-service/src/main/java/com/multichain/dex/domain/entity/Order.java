...
public class Order {
    ...
    /**
     * Counterparty addresses follow a symmetric 4-endpoint model for a swap:
     * {creator, matcher} × {source registry A, target registry B}. They map onto the two HTLCs as
     * <pre>
     *   HTLC@A: initiator = creatorSourceAddress, participant = matcherSourceAddress
     *   HTLC@B: initiator = matcherTargetAddress, participant = creatorTargetAddress
     * </pre>
     * Source-side addresses come from on-chain events (indexer); the full target-side addresses
     * (which may not fit on-chain, e.g. 32-byte SUI in a 20-byte EVM field) are attached off-chain.
     */

    /** Creator's address on the source registry (= creator@A). */
    @Column(name = "creator_source_address", nullable = false, length = 200)
    private String creatorSourceAddress;

    /** Matcher's address on the source registry (= matcher@A); null until matched. */
    @Column(name = "matcher_source_address", length = 200)
    private String matcherSourceAddress;
    ...
    /** Creator's receiving address on the target registry (= creator@B); full address, attached off-chain. */
    @Column(name = "creator_target_address", length = 200)
    private String creatorTargetAddress;

    /** Matcher's address on the target registry (= matcher@B); full address, attached off-chain at match time. */
    @Column(name = "matcher_target_address", length = 200)
    private String matcherTargetAddress;

    /** Creator's notification email (opt-in, off-chain only, never exposed in public API). */
    @Column(name = "creator_email", length = 255)
    private String creatorEmail;

    /** Matcher's notification email (opt-in, off-chain only, never exposed in public API). */
    @Column(name = "matcher_email", length = 255)
    private String matcherEmail;
