...
public class EvmChainScanner implements ChainScanner {
    ...
    /** Total attempts = MAX_RETRIES + 1 (initial call). Higher to absorb rate-limit (429) spikes. */
    private static final int MAX_RETRIES = 5;
    /** Base delay for non-rate-limit transient errors. */
    private static final long BASE_DELAY_MS = 300L;
    /** Base delay specifically for HTTP 429 — Alchemy free tier needs a longer cool-down. */
    ...
    // Package-visible for unit testing the order-upsert logic with mocked repositories.
    void upsertSameChainOrder(Chain chain, String orderId, Map<String, Object> data) {
        ...
        Order order = existing.orElseGet(() -> Order.builder()
                ...
                .creatorSourceAddress((String) data.get("creator"))
    ...
    // Package-visible for unit testing (insert/update paths + off-chain-address invariant).
    void upsertCrossChainOrder(Chain chain, String orderId, Map<String, Object> data) {
        ...
        Order order = existing.orElseGet(() -> Order.builder()
                ...
                .creatorTargetAddress((String) data.get("targetAddress"))
                .build());
        ...
        if (matchedBy != null && !matchedBy.equals("0x0000000000000000000000000000000000000000") && order.getMatcherSourceAddress() == null) {
    ...
    private <T> T retryRpc(RpcCall<T> call) throws Exception {
        ...
        for (int i = 0; i <= MAX_RETRIES; i++) {
            try {
                ...
                long delay = backoffDelay(e, i);
                if (isRateLimited(e)) {
                    log.debug("[EVM] Rate-limited (429), backing off {}ms (attempt {}/{})", delay, i + 1, MAX_RETRIES);
                }
                Thread.sleep(delay);
            }
        }
    ...
    /** Exponential backoff with jitter; longer cool-down specifically for HTTP 429. */
    private long backoffDelay(Exception e, int attempt) {
        long base = isRateLimited(e) ? RATE_LIMIT_BASE_DELAY_MS : BASE_DELAY_MS;
        long delay = Math.min(base * (1L << attempt), MAX_DELAY_MS);
        long jitter = (long) (Math.random() * (delay / 2.0));
        return delay + jitter;
    }
    ...
    String resolveTargetChainId(BigInteger numericChainId) {
