...
contract HTLC is ReentrancyGuard {
    ...
    function createSwap(
        ...
        if (_token == address(0)) {
            ...
            if (msg.value > 0) revert InvalidAmount(); // Don't accept ETH for ERC20 swaps
    ...
    /**
     * @notice Check if a swap exists and is active
     * @param _swapId Swap identifier
     */
    ...
    /**
     * @notice Calculate hashlock from secret (helper function)
     * @param _secret The secret to hash
     */
    ...
    /**
     * @notice Get all swap IDs where user is initiator
    ...
    /**
     * @notice Get all swap IDs where user is participant
     * @param _user User address
     */
    ...
    /**
     * @notice Generate a unique swap ID
     * @param _initiator Initiator address
     * @param _participant Participant address
     * @param _hashlock The hashlock
     * @param _timelock The timelock
     */
     ...
     * createSwap(). Uses abi.encode (32-byte padded fields) so it matches the off-chain
     * frontend derivation byte-for-byte (encodeAbiParameters with address/bytes32/uint256 types).
     */
