...
async function main() {
  ...
  const { packageId } = deploymentInfo;
  ...
  if (!tkaCapObject || !tkbCapObject) {
    console.error('❌ TreasuryCap objects not found. Make sure tokens were deployed correctly.');
    process.exit(1);
  }
  ...
  const mintAmount = 1_000_000_000_000; // 1000 tokens (9 decimals)
  console.log(`💰 Minting ${mintAmount / 1_000_000_000} tokens to ${address}...\n`);

  // Mint TKA
  console.log('1. Minting sTKA...');
  const mintTkaTx = new Transaction();
  mintTkaTx.moveCall({
    target: `${packageId}::test_token_a::mint`,
    arguments: [
      mintTkaTx.object(tkaCapId),
      mintTkaTx.pure.u64(mintAmount),
      mintTkaTx.pure.address(address),
    ],
  });

  const tkaMintResult = await client.signAndExecuteTransaction({
    signer: keypair,
    transaction: mintTkaTx,
  });
  console.log(`   ✅ Transaction: ${tkaMintResult.digest}`);

  // Mint TKB
  console.log('\n2. Minting sTKB...');
  const mintTkbTx = new Transaction();
  mintTkbTx.moveCall({
    target: `${packageId}::test_token_b::mint`,
    arguments: [
      mintTkbTx.object(tkbCapId),
      mintTkbTx.pure.u64(mintAmount),
      mintTkbTx.pure.address(address),
    ],
  });

  const tkbMintResult = await client.signAndExecuteTransaction({
    signer: keypair,
    transaction: mintTkbTx,
  });
