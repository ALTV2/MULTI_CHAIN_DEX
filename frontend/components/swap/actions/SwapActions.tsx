...
import type { DetectedHTLC, SwapActionPanelProps } from '../SwapActionPanel';

/**
 * Validate that a secret matches the expected hashlock before sending an on-chain transaction.
 * Returns an error message if invalid, or null if valid.
 */
export function validateSecretAgainstHashlock(secret: string, hashlock: string | undefined): string | null {
...
export function WaitingMessage({ text }: { text: string }) {
  return (
...
export function OrderCreatedAction({ swap, onUpdate, detectedHTLC }: { swap: ActiveSwap; onUpdate?: () => void; detectedHTLC?: DetectedHTLC | null }) {
 ...
 */
export function CreateCounterHTLCAction({
  swap,
  detectedHTLC,
  onUpdate,
}: {
 ...
 */
export function CreatorLockAction({ swap, onUpdate }: { swap: ActiveSwap; onUpdate: () => void }) {
  const { address } = useAccount();
  const publicClient = usePublicClient();
  ...
  const secretStorage = useSettingsStore((s) => s.secretStorage);
  const isShowOnce = secretStorage === 'show_once';

  // Generate secret once for this component instance.
  // Persistence depends on settings: 'local' = saved to localStorage; 'show_once' = stripped before save.
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Secret — display depends on storage mode */}
      {step !== 'done' && !isSuccess && (
        <div className={`p-3 rounded-xl border space-y-2 ${
          ...
          <p className={`text-xs font-semibold ${isShowOnce ? 'text-red-400' : 'text-yellow-400'}`}>
            {isShowOnce
              ? '⚠️ This secret will NOT be saved. Save it now — without it you cannot claim your funds.'
              : '⚠️ Save your secret as backup — it is stored locally but may be lost if you clear browser data!'}
          </p>
          <p className="text-xs text-gray-400">
            {isShowOnce
              ? 'You will need to paste this secret manually when claiming. There is no recovery.'
              : 'The secret will be auto-filled when claiming, but keep a copy just in case.'}
          </p>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              className={`w-4 h-4 ${isShowOnce ? 'accent-red-400' : 'accent-yellow-400'}`}
            />
            <span className="text-xs text-gray-300">
              {isShowOnce
                ? 'I have saved this secret in a secure location'
                : 'I have saved my secret in a safe place'}
            </span>
          </label>
        </div>
      )}
 ...
 */
export function MatcherLockAction({ swap, onUpdate }: { swap: ActiveSwap; onUpdate: () => void }) {
  const { address } = useAccount();
  const suiAccount = useCurrentAccount();
  const { meta } = swap;
  ...
  const handleLock = async () => {
    ...
    try {
      ...
      if (isSuiTarget) {
        ...
        // For EVM→SUI: the creator's full 32-byte SUI address is attached off-chain at
        // order creation (POST /orders/metadata) and surfaced by the API as the order's
        // targetAddress, which flows into meta.creatorSuiAddress / meta.targetAddress.
 ...
 */
export function CreatorWithdrawAction({ swap, onUpdate, detectedHTLC }: { swap: ActiveSwap; onUpdate: () => void; detectedHTLC?: DetectedHTLC | null }) {
  const { address } = useAccount();
  const suiAccount = useCurrentAccount();
  const { meta } = swap;
  const [error, setError] = useState<string | null>(null);
 ...
 */
export function MatcherWithdrawAction({ swap, onUpdate }: { swap: ActiveSwap; onUpdate: () => void }) {
  const { address } = useAccount();
  const { meta } = swap;
  const [error, setError] = useState<string | null>(null);
 ...
 */
export function SuiSourceMatcherWithdrawAction({ swap, onUpdate }: { swap: ActiveSwap; onUpdate: () => void }) {
  const { address } = useAccount();
  const suiAccount = useCurrentAccount();
  const { meta } = swap;
  const [error, setError] = useState<string | null>(null);
 ...
 */
export function SuiSourceCreatorClaimEvmAction({ swap, onUpdate }: { swap: ActiveSwap; onUpdate: () => void }) {
  const { address } = useAccount();
  const { meta } = swap;
  const [error, setError] = useState<string | null>(null);
 ...
 */
export function formatTimelockRelative(timelock?: bigint): { label: string; expired: boolean } | null {
  if (!timelock) return null;
  const now = Math.floor(Date.now() / 1000);
  const diff = Number(timelock) - now;
  if (diff <= 0) return { label: 'Refund window open', expired: true };
  const h = Math.floor(diff / 3600);
  const m = Math.floor((diff % 3600) / 60);
  if (h >= 24) {
    const d = Math.floor(h / 24);
    return { label: `Refundable in ${d}d ${h % 24}h`, expired: false };
  }
  if (h >= 1) return { label: `Refundable in ${h}h ${m}m`, expired: false };
  return { label: `Refundable in ${m}m`, expired: false };
}

export function RefundAction({ swap, onUpdate }: { swap: ActiveSwap; onUpdate: () => void }) {
  const { address } = useAccount();
  const suiAccount = useCurrentAccount();
  const evmChainId = useChainId();
  const { switchChainAsync } = useSwitchChain();
  ...
  const timelock = meta.role === 'creator' ? swap.creatorHtlcTimelock : swap.matcherHtlcTimelock;
  const timelockInfo = formatTimelockRelative(timelock);

  // Refresh the countdown label every 30s while the component is mounted
  useEffect(() => {
    if (!timelock || timelockInfo?.expired) return;
    const id = setInterval(() => setTick((t) => t + 1), 30_000);
    return () => clearInterval(id);
  }, [timelock, timelockInfo?.expired]);
  ...
  const handleRefund = async () => {
    ...
    try {
      if (isSuiRefund) {
        ...
        const targetChain = refundChainId as number;
        if (evmChainId !== targetChain) {
          setIsSwitching(true);
          try {
  ...
  const needsChainSwitch = !isSuiRefund && typeof refundChainId === 'number' && evmChainId !== refundChainId;
    ...
    <div className="space-y-3">
      {timelockInfo && (
        <div className={`p-2 rounded text-xs ${
          ...
          {timelockInfo.expired ? '⏰ ' : '⏳ '}
          {timelockInfo.label}
          {!timelockInfo.expired && <span className="text-gray-400 ml-1">(refund unavailable until then)</span>}
        </div>
      )}
      ...
      {isSuccess ? (
        ...
        <Button
          size="sm"
          variant="danger"
          loading={isPending || isConfirming || isSwitching}
          onClick={handleRefund}
          disabled={
            isPending || isConfirming || isSwitching ||
            (isSuiRefund && !suiAccount) ||
            (timelockInfo !== null && !timelockInfo.expired)
          }
        >
          {isSwitching ? 'Switching network...'
            : isPending || isConfirming ? 'Refunding...'
            : needsChainSwitch ? 'Switch network & Refund'
            : 'Refund Tokens'}
        </Button>
      )}
    </div>
 ...
 */
export function SuiSourceCreatorCounterHTLCAction({ swap, onUpdate }: { swap: ActiveSwap; onUpdate: () => void }) {
  const { address } = useAccount();
  const publicClient = usePublicClient();
  const suiAccount = useCurrentAccount();
  const { meta } = swap;
  ...
  }, [meta.matcherHtlcSwapId, targetChainId]);
 ...
 */
export function CompleteAction({ swap, onUpdate }: { swap: ActiveSwap; onUpdate: () => void }) {
  const { meta } = swap;
  const [error, setError] = useState<string | null>(null);
