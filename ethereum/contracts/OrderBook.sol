...
contract OrderBook is ReentrancyGuard, Ownable {
    ...
    bool public restrictTokens;                 // Флаг ограничения токенов вне списка доступных (по умолчанию false)

    enum OrderStatus {
        Active,    // Активный(после создания)
        Pending,   // Ожидает завершения
        Completed, // Успешно исполнен
        Cancelled  // Отменен
    }
    ...
    event OrderExecuted(uint256 indexed id, uint256 timestamp);
    ...
    modifier onlyTradeContract() {
        require(msg.sender == tradeContract, "Only Trade contract can call this method");
        _;
    }
