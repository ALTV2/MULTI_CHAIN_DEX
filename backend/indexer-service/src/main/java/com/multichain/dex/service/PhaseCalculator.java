...
public class PhaseCalculator {
    ...
    public SwapPhase compute(Order order) {
        ...
        if (order.getOrderType() == OrderType.SAME_CHAIN) {
            ...
            if (isOrderExpired(order)) return SwapPhase.REFUNDED;
            if (order.getMatcherSourceAddress() != null) return SwapPhase.ORDER_MATCHED;
        ...
        // Order matched but no HTLCs yet — no funds locked, so expired = terminal
        if (order.getMatcherSourceAddress() != null || order.getStatus() == OrderStatus.MATCHED) {
            if (isOrderExpired(order)) return SwapPhase.REFUNDED;
            return SwapPhase.ORDER_MATCHED;
        }

        // Expired order without any HTLCs
        if (order.getStatus() == OrderStatus.EXPIRED || isOrderExpired(order)) {
            return SwapPhase.REFUNDED;
        }
    ...
    private boolean isOrderExpired(Order order) {
        return order.getExpiresAt() != null && order.getExpiresAt().isBefore(Instant.now());
    }
}
